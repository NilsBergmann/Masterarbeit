...
import bergmann.masterarbeit.generationtarget.expressions.StringDomainValue;
...
class StateInformationTest {
	...
	void test() {
		for (int i = 0; i < 100; i++) {
			...
			current.storeDomainValue("Test", Optional.of(test));
			states.add(current);
		}
		assertEquals(100, states.getAllStates().size());

		Expression getTestExpr = new NumberDomainValue("Test");
		Expression getWrongType = new StringDomainValue("Test");
		Expression getUndefinedValueExpr = new BooleanDomainValue("Undefined");
		...
		for (State s : states.getAllStates()) {
			...
			// After evaluating a UserVariable once, there should be a value stored for it
			// in the state
