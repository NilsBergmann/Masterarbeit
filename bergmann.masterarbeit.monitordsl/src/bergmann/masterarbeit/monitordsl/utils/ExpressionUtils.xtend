...
class ExpressionUtils {
	...
	def static String getShortName(Expression expr){
		switch expr{
			...
			And: return "And"
			Or: return "Or"
			LTL_Binary: return expr.op.toString
			LTL_Unary: return expr.op.toString
			Rel: return expr.op.toString
			Add: return expr.op.toString
			Mult: return expr.op.toString
			Negation: return "Negation"
			Subexpression: return expr.expr.getShortName
			AggregateExpression: return expr.op.toString
			IntLiteral: return "Integer_Literal"
			FloatLiteral: return "Float_Literal"
			BoolLiteral: return "Boolean_Literal"
			StringLiteral: return "String_Literal"
			CrossReference: switch expr.ref {
			...
			MappingBinary: return (expr.ref as BinaryJava).name
			TimeOffset: return "Offset"
			IfThenElse: return "If-Then-Else"
			default: return expr.toString
		}
	}
}
